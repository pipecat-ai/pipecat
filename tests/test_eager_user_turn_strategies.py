...
class TestEagerUserTurnStrategies(unittest.IsolatedAsyncioTestCase):
    async def test_eager_end_of_turn_runs_inference_without_touching_the_context(self):
        ...
        assert provisional.speculation
    ...
    async def test_matching_transcript_ends_the_turn_and_keeps_the_response(self):
        ...
        assert contexts[0].speculation
        # The turn end is what releases the held response.
        assert len(stops) == 1
    ...
    async def test_differing_transcript_withdraws_the_speculation(self):
        ...
        assert [c.speculation for c in contexts] == [True, False]
        assert isinstance(cancel, EagerEndOfTurnCancelFrame)
    ...
    async def test_resuming_the_turn_withdraws_the_speculation(self):
        ...
        # its own, since the prediction was already withdrawn.
        assert len(cancels) == 1
        assert contexts[0].speculation
        assert not contexts[1].speculation
    ...
    async def test_turn_without_an_eager_prediction_behaves_normally(self):
        ...
        assert not any(isinstance(f, EagerEndOfTurnCancelFrame) for f in down)
        assert len(contexts) == 1
        assert not contexts[0].speculation
        assert any(isinstance(f, UserStoppedSpeakingFrame) for f in down)
...
class TestSpeculativeToolCalls(unittest.IsolatedAsyncioTestCase):
    async def test_speculative_inference_does_not_execute_tools(self):
        ...
        speculative = LLMContextFrame(context=context, speculation=True)
...
class TestUnresolvedSpeculation(unittest.IsolatedAsyncioTestCase):
    async def test_new_turn_withdraws_a_speculation_left_in_flight(self):
        ...
        strategy.add_event_handler(
            "on_user_turn_speculation_cancelled", lambda s: withdrawals.append(s)
        )
...
class TestTurnCommittedWithoutATranscript(unittest.IsolatedAsyncioTestCase):
    async def test_speculation_is_withdrawn_when_there_is_nothing_to_compare(self):
        ...
        assert [c.speculation for c in contexts] == [True]
        assert len(cancels) == 1
...
class TestDeferredEagerStrategy(unittest.IsolatedAsyncioTestCase):
    async def test_deferring_carries_the_speculation_to_the_subscriber(self):
        ...
        await inner.setup(frame_processor_setup(TaskManager()))
...
class TestLLMServiceHoldsTheSpeculation(unittest.IsolatedAsyncioTestCase):
    ...
    @staticmethod
    def context_frame(speculation: bool = False):
        context = LLMContext(messages=[{"role": "user", "content": "book a flight"}])
    ...
    async def test_a_speculative_response_leaves_the_service_only_once_confirmed(self):
        down, _ = await run_test(
            SpeculativeLLM(),
            frames_to_send=[
                self.context_frame(speculation=True),
                SleepFrame(),
                UserStoppedSpeakingFrame(),
                SleepFrame(),
            ],
...
class TestATurnOutlastingTheHoldIsStillAnswered(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_a_matching_commit_after_the_hold_expires_runs_a_fresh_inference(self):
        ...
        assert context.messages == [{"role": "user", "content": "Book a flight."}]
        # Withdrawn where every other discard is withdrawn, so the gate drops
        # the held response rather than relying on a later one to supersede it.
        assert len([f for f in down if isinstance(f, EagerEndOfTurnCancelFrame)]) == 1
...
class EchoLLM(LLMService):
    """Answers with whatever transcript it was handed, so stale answers are visible."""
    ...
    async def process_frame(self, frame, direction):
        ...
        await self.push_frame(LLMFullResponseStartFrame())
        await self.push_frame(LLMTextFrame(f"ANSWER[{frame.context.messages[-1]['content']}]"))
        await self.push_frame(LLMFullResponseEndFrame())


class TestAWithdrawnResponseIsNeverSpoken(unittest.IsolatedAsyncioTestCase):
    """A withdrawal has to reach the gate before the turn end that follows it.

    Both travel from the strategy to the same gate. If the withdrawal took a
    slower route it would arrive second, and the turn end would release the very
    response the withdrawal was meant to discard.
    """

    async def test_a_mismatched_transcript_never_speaks_the_prediction(self):
        context = LLMContext()

        down, _ = await run_test(
            Pipeline([aggregator(context), EchoLLM()]),
            frames_to_send=[
                ProposedUserStartedSpeakingFrame(),
                SleepFrame(),
                eager("i want to cancel"),
                SleepFrame(),
                # The user carried on, so the committed transcript differs.
                final("i want to cancel, actually reschedule it"),
                SleepFrame(),
                ProposedUserStoppedSpeakingFrame(),
                SleepFrame(sleep=0.8),
        ...
        # Only the committed transcript is answered — not the sentence the user
        # abandoned, and not both.
        assert [f.text for f in down if isinstance(f, LLMTextFrame)] == [
            "ANSWER[i want to cancel, actually reschedule it]"
        ]

        order = [
            type(f).__name__
            for f in down
            if isinstance(f, (EagerEndOfTurnCancelFrame, UserStoppedSpeakingFrame))
        ]
        assert order == ["EagerEndOfTurnCancelFrame", "UserStoppedSpeakingFrame"]
...
class TestToolCallsAcrossTheConfirmation(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_a_call_reached_before_the_turn_ends_is_withdrawn(self):
        ...
        down, up = await run_test(
            llm,
            frames_to_send=[self.context_frame(speculation=True), SleepFrame(sleep=0.8)],
        )
