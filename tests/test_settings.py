...
class TestServiceSettings:
    ...
    def test_given_fields_includes_extras(self):
        s = ServiceSettings(model="gpt-4o")
        s.extras = {"custom_key": 42}
    ...
    def test_copy_is_deep(self):
        ...
        assert c.extras == {"nested": {"a": 1}}
        # Mutating the copy shouldn't affect the original
        c.extras["nested"]["a"] = 999
        assert s.extras["nested"]["a"] == 1
...
class TestApplyUpdate:
    ...
    def test_apply_update_extras_merged(self):
        current = TTSSettings(voice="alice")
        current.extras = {"speed": 1.0, "stability": 0.5}
        delta = TTSSettings()
        delta.extras = {"speed": 1.2}
        changed = current.apply_update(delta)
        assert "speed" in changed
        assert current.extras == {"speed": 1.2, "stability": 0.5}

    def test_apply_update_extras_no_change(self):
        current = TTSSettings(voice="alice")
        current.extras = {"speed": 1.0}
        delta = TTSSettings()
        delta.extras = {"speed": 1.0}
        changed = current.apply_update(delta)
        assert changed == set()
...
class TestFromMapping:
    ...
    def test_unknown_keys_go_to_extras(self):
        ...
        assert s.voice == "alice"
        assert s.extras == {"speed": 1.2, "stability": 0.5}
    ...
    def test_llm_settings_from_mapping(self):
        ...
        assert s.extras == {"custom_param": True}
...
class TestRoundtrip:
    def test_from_mapping_then_apply_update(self):
        ...
        current.extras = {"stability": 0.5, "speed": 1.0}
        ...
        assert current.voice == "bob"
        assert current.language == "en"
        assert current.extras["speed"] == 1.2
        assert current.extras["stability"] == 0.5
