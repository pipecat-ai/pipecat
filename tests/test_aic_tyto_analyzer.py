...
class TestAICTytoAnalyzer(unittest.IsolatedAsyncioTestCase):
    ...
    def _make(self, **kwargs):
        ...
        analyzer = self.AICTytoAnalyzer(**analyzer_kwargs)
        self.addAsyncCleanup(analyzer.cleanup)
        return analyzer
    ...
    def test_buffer_reinitializes_on_sample_rate_change(self):
        ...
        self.assertEqual(len(self.mock_collector.initialized_with), 2)
        self.assertEqual(analyzer._buffered_samples, 80)
    ...
    async def test_analyze_once_emits_metrics_frame(self):
        ...
        analyzer._buffered_samples = 80000
    ...
    def test_invalid_interval(self):
        for value in (0, -1, float("nan"), float("inf")):
            with self.subTest(value=value), self.assertRaises(ValueError):
                self._make(analysis_interval=value)

    async def test_full_window_and_fresh_audio_required(self):
        analyzer = self._make()
        analyzer.push_frame = AsyncMock()
        analyzer._buffer_audio(self._audio_frame(num_samples=79999))
        await analyzer._analyze_once()
        self.assertEqual(self.mock_analyzer.analyze_calls, 0)
        analyzer._buffer_audio(self._audio_frame(num_samples=1))
        await analyzer._analyze_once()
        await analyzer._analyze_once()
        self.assertEqual(self.mock_analyzer.analyze_calls, 1)
        analyzer.reset()
        await analyzer._analyze_once()
        self.assertEqual(self.mock_analyzer.analyze_calls, 1)

    async def test_input_filter_collects_before_enhancement_without_double_counting(self):
        from pipecat.processors.frame_processor import FrameDirection, FrameProcessor

        analyzer = self._make()
        enhancement = MagicMock()
        enhancement.start = AsyncMock()
        enhancement.stop = AsyncMock()
        enhancement.filter = AsyncMock(return_value=b"\x00\x00" * 160)
        adapter = analyzer.as_input_filter(enhancement)
        await adapter.start(16000)
        original = self._audio_frame(num_samples=160)
        enhanced = await adapter.filter(original.audio)
        np.testing.assert_allclose(self.mock_collector.buffer_calls[0], np.arange(160) / 32768.0)
        self.assertEqual(enhanced, b"\x00\x00" * 160)
        analyzer.push_frame = AsyncMock()
        with patch.object(FrameProcessor, "process_frame", new_callable=AsyncMock):
            await analyzer.process_frame(original, FrameDirection.DOWNSTREAM)
        self.assertEqual(len(self.mock_collector.buffer_calls), 1)
        await adapter.stop()
        enhancement.stop.assert_awaited_once()

    async def test_cleanup_waits_for_native_inference_before_termination(self):
        analyzer = self._make()
        analyzer._buffer_audio(self._audio_frame(num_samples=80000))
        ...
        self.mock_analyzer.analyze_buffered = inference
        self.mock_analyzer.terminate_session = terminated.set
        analyzer.push_frame = AsyncMock()
        task = asyncio.create_task(analyzer._analyze_once())
        analyzer._analysis_task = task
        while not entered.is_set():
            await asyncio.sleep(0.001)

        # Match TaskManager cancellation without requiring a running pipeline.
        async def cancel(task):
            task.cancel()
            await asyncio.gather(task, return_exceptions=True)

        analyzer.cancel_task = cancel
        cleanup = asyncio.create_task(analyzer.cleanup())
    ...
    async def test_reset_discards_in_flight_result(self):
        analyzer = self._make()
        analyzer._buffer_audio(self._audio_frame(num_samples=80000))
        analyzer.push_frame = AsyncMock()
        self.mock_analyzer.analyze_buffered = lambda: (analyzer.reset(), MockAnalysisResult())[1]
        await analyzer._analyze_once()
        analyzer.push_frame.assert_not_called()
