...
class TestAggregatedTextProgressFrame(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_sentence_carrying_ssml_still_reports_progress(self):
        ...
        Both spellings are checked. The flush form is the harder one: stripping
        the tag alone still loses a word of progress and runs "work." into
        "Take", because the tag also has to be isolated on the TTS cursor and the
        space it stood in for kept on the caption side.
        """
        steps = [
