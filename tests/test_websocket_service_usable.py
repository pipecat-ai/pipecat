...
class FakeWebsocketService(AIService, WebsocketService):
    ...
    async def report_error(self, error: ErrorFrame, processor_became_unusable: bool = False):
...
class TestReconnection(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_only_giving_up_reports_the_service_as_spent(self):
        ...
        async def report_error(error, processor_became_unusable=False):
            flags.append(processor_became_unusable)
            await service.push_error_frame(
                error, processor_became_unusable=processor_became_unusable
            )
