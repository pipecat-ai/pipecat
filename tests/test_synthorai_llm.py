...
"""Unit tests for Synthorai LLM service."""
...
from pipecat.services.synthorai.llm import SynthoraiLLMService


@pytest.mark.asyncio
async def test_synthorai_llm_stream_closed_on_cancellation():
    ...
    """
    with patch.object(SynthoraiLLMService, "create_client"):
        service = SynthoraiLLMService(
            api_key="test-key", settings=SynthoraiLLMService.Settings(model="test-model")
        )
