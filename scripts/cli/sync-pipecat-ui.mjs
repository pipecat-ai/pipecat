...
// per-release payloads, so the snapshot is built from the pipecat-ui repository
// instead: it is cloned at a release tag (or any ref), its registry JSON is built
// with the shadcn CLI and served locally for the install. dependencies.json
...
/** Highest semver tag of the form vX.Y.Z in the pipecat-ui repository. */
async function latestTag(repository) {
  const refs = await run("git", ["ls-remote", "--tags", "--refs", repository], repoRoot, {
    capture: true,
  });
  const tags = refs
    .split("\n")
...
try {
  ...
  // The @pipecat items come from a registry built at the chosen ref and served
  // locally, so the snapshot is tied to a commit.
  const repository = path.isAbsolute(options.repo) || options.repo.startsWith(".")
    ? path.resolve(options.repo)
    : options.repo;
  const ref = options.ref ?? options.tag ?? (await latestTag(repository));
  ...
  const source = { repository, ref, commit };
  ...
  componentsJson.registries = { ...componentsJson.registries, "@pipecat": registryServer.url };
