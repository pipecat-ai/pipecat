...
"""Write the JSON Schema for ``FlowConfig`` next to the model.

The schema is kept in the repository as
``src/pipecat/flows/flow_config.schema.json`` so the Flows editor and other
tools can vendor it at a pinned version. ``tests/test_flows_config.py`` fails
when the file drifts from the model; rerun this script to refresh it::
