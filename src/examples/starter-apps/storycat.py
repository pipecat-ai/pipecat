...
class StoryImageGenerator(AIService):
    ...
    async def process_frame(self, frame: Frame) -> AsyncGenerator[Frame, None]:
        if isinstance(frame, StoryPageFrame):
            ...
            image_prompt = ""
            async for frame in self._llm.process_frame(LLMMessagesQueueFrame(msgs)):
                if isinstance(frame, TextFrame):
                    image_prompt += frame.text
...
async def main(room_url: str, token):
    async with aiohttp.ClientSession() as session:
        ...
        sig = StoryImageGenerator(story, llm)
        ...
        async def storytime():
            ...
            pipeline = Pipeline(
                processors=[ura, llm, sp, fl, ParallelPipeline([[tts], [sig]]), lra]
            )
