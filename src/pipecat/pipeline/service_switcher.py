...
class ServiceSwitcherStrategy(BaseObject):
    ...
    - on_service_switched: Called synchronously when the active service changes.
    ...
    def __init__(self, services: List[FrameProcessor]):
        ...
        self._register_event_handler("on_service_switched", sync=True)
    ...
    @abstractmethod
    async def handle_frame(self, frame: ServiceSwitcherFrame, direction: FrameDirection):
...
class ServiceSwitcherStrategyManual(ServiceSwitcherStrategy):
    ...
    async def handle_frame(self, frame: ServiceSwitcherFrame, direction: FrameDirection):
        ...
        if isinstance(frame, ManuallySwitchServiceFrame):
            await self._set_active_if_available(frame.service)
        else:
            raise ValueError(f"Unsupported frame type: {type(frame)}")

    async def _set_active_if_available(self, service: FrameProcessor):
...
class ServiceSwitcher(ParallelPipeline, Generic[StrategyType]):
    ...
    def __init__(self, services: List[FrameProcessor], strategy_type: Type[StrategyType]):
        ...
        self.strategy.add_event_handler("on_service_switched", self._on_service_switched)
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        if isinstance(frame, ServiceSwitcherFrame):
            await self.strategy.handle_frame(frame, direction)

    async def _on_service_switched(self, _: ServiceSwitcherStrategy, service: FrameProcessor):
        # Request metadta directly to the new active service.
