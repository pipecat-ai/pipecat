...
def _chatgpt_account_id_from_access_token(access_token: str) -> str | None:
    """Extract the ChatGPT account ID claim from a Codex OAuth access token.

    Codex ChatGPT auth access tokens are JWTs whose payload may contain the
    account ID under ``https://api.openai.com/auth.chatgpt_account_id``. The
    header is optional for some endpoints, so malformed/non-JWT tokens simply
    return ``None`` and are left to fail through the normal auth path.
    """
...
def _codex_realtime_headers(access_token: str) -> dict[str, str]:
    """Build Codex CLI-compatible headers for ChatGPT OAuth auth.

    These headers mirror the first-party Codex CLI shape used against the
    ChatGPT Codex backend. Realtime support with ChatGPT/Codex auth is
    experimental and endpoint/entitlement dependent; callers must still provide
    a compatible ``base_url`` and model.
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Pipecat)",
        "originator": "codex_cli_rs",
    }
    account_id = _chatgpt_account_id_from_access_token(access_token)
    if account_id:
...
def _codex_auth_file() -> Path:
    ...
    Codex stores file-backed auth at ``$CODEX_HOME/auth.json``. When
    ``CODEX_HOME`` is unset, Codex defaults it to ``~/.codex`` on macOS,
    Linux, and Windows.
    """
...
def _codex_access_token_from_auth_file(path: Path) -> str | None:
    """Read a Codex OAuth access token from an auth file."""
    try:
        with path.open() as f:
            auth = json.load(f)
...
def _load_codex_access_token() -> str:
    """Load a Codex OAuth access token from the local Codex auth file."""
    if access_token := _codex_access_token_from_auth_file(_codex_auth_file()):
        return access_token

    raise ValueError(
        "Codex auth token not found. Run `codex login` and then retry, or pass `api_key` "
        "explicitly."
    )
...
class OpenAIRealtimeLLMService(LLMService[OpenAIRealtimeLLMAdapter]):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            api_key: OpenAI API key for authentication. When ``use_codex_auth`` is
                enabled and ``api_key`` is not provided, the access token is loaded
                from the local Codex auth file.
            ...
            use_codex_auth: Add Codex CLI-compatible headers for ChatGPT OAuth.
                If ``api_key`` is not provided, the access token is loaded from the
                local Codex auth file. This is experimental and requires a compatible
                Realtime endpoint/model entitlement.
        ...
        if use_codex_auth and api_key is None:
            api_key = _load_codex_access_token()
        if api_key is None:
            raise ValueError("api_key is required unless use_codex_auth is enabled.")

        self.api_key = api_key
        self.base_url = full_url
        self._auth_headers = _codex_realtime_headers(api_key) if use_codex_auth else {}
    ...
    def _websocket_headers(self) -> dict[str, str]:
        """Return headers for the Realtime WebSocket connection."""
    ...
    async def _connect(self):
        try:
            ...
            self._websocket = await websocket_connect(
                uri=self.base_url,
                additional_headers=self._websocket_headers(),
            )
