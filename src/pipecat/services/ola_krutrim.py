...
def language_to_ola_krutrim_language(language: Language) -> str | None:
    # Implement language mapping when Ola Krutrim supports different languages
...
class OlaKrutrimTTSService(TTSService):
    ...
    async def set_language(self, language: Language):
        # Implement when Krutrim supports language selection
        pass
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, headers=headers, json=payload) as resp:
                    if resp.status == 200:
                        ...
                        logger.error(
                            f"Failed to get a response. Status code: {resp.status}"
                        )
