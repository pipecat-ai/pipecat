...
def language_to_ola_krutrim_language(language: Language) -> str | None:
    # For now, krutrim only supports english.This may be used in future
    return None
...
class OlaKrutrimTTSService(TTSService):
    ...
    async def set_model(self, model: str):
        #presently Krutrim supports only one model
        logger.debug(f"Switching TTS model to: [{model}]")
        self._model_name = model
        await super().set_model(model)

    async def set_language(self, language: Language):
        # Implement if Ola Krutrim supports language selection in future
        pass
