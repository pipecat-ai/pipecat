...
Adapted for the feat/jambonz-integration branch's existing base classes.
"""
...
class AgentInputParams(BaseModel):
    ...
    Parameters:
        ...
        agent_id: The ID of the Ultravox Realtime agent.
        template_context: Context variables for the agent call.
        metadata: Metadata to attach to the call.
        output_medium: Initial output medium ("text" or "voice").
        max_duration: Maximum duration of the call.
        extra: Extra parameters for the agent call creation request.
    """
...
class OneShotInputParams(BaseModel):
    ...
    Parameters:
        ...
        voice: Voice identifier for speech generation.
        metadata: Metadata to attach to the call.
        output_medium: Initial output medium ("text" or "voice").
        max_duration: Maximum duration of the call.
        extra: Extra parameters for the call creation request.
    """
...
class UltravoxRealtimeLLMService(LLMService):
    ...
    def __init__(
        ...
        *,
        ...
        selected_tools: Optional[ToolsSchema] = None,
        **kwargs,
    ):
        ...
        Args:
            ...
            selected_tools: ToolsSchema for tools to use with OneShotInputParams calls.
        ...
        if selected_tools:
            if not isinstance(self._params, OneShotInputParams):
                logger.warning(
                    "selected_tools may only be set when using OneShotInputParams; ignoring."
                )
            else:
    ...
    async def start(self, frame: StartFrame):
        ...
        try:
            ...
            await self.push_error(ErrorFrame(error=f"Failed to connect to Ultravox: {e}"))
    ...
    async def set_output_medium(self, output_medium: str):
        """Set the output medium for the Ultravox Realtime call.

        Args:
            output_medium: The output medium ("voice" or "text").
        """
        output_medium = output_medium.lower()
        if output_medium == "audio":
            output_medium = "voice"
        if output_medium not in {"voice", "text"}:
    ...
    async def _send(self, content: Union[bytes, Dict[str, Any]]):
        ...
        try:
            ...
            if self._disconnecting or not self._socket:
                return
            logger.error(f"Ultravox websocket send error: {e}")
            await self.push_error(ErrorFrame(error=f"Ultravox websocket send error: {e}"))
    ...
    async def _receive_messages(self):
        ...
        async for message in self._socket:
            try:
                ...
                if self._disconnecting or not self._socket:
                    return
                logger.error(f"Ultravox websocket receive error: {e}")
                await self.push_error(
                    ErrorFrame(error=f"Ultravox websocket receive error: {e}")
                )
    ...
    async def _handle_agent_transcript(
        ...
        if medium == "voice":
            ...
            if delta:
                await self.push_frame(TTSTextFrame(text=delta))
        elif medium == "text":
            if final:
                await self.stop_processing_metrics()
                await self.push_frame(LLMFullResponseEndFrame())
    ...
    def create_context_aggregator(
        ...
        *,
        ...
        lc = LLMContext.from_openai_context(context) if hasattr(LLMContext, "from_openai_context") else context
        assistant_params.expect_stripped_words = False
        return LLMContextAggregatorPair(
            lc, user_params=user_params, assistant_params=assistant_params
        )
