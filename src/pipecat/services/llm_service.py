...
# `WebsocketLLMService` and `WebsocketReconnectedError` moved to
# `pipecat.services.websocket_llm_service` so that importing this module does
# not require the optional `websockets` package. Re-export them lazily here to
# preserve the `pipecat.services.llm_service` import path; accessing them still
# pulls in `websockets` (as it must, since they depend on it).
_MOVED_TO_WEBSOCKET_MODULE = frozenset({"WebsocketLLMService", "WebsocketReconnectedError"})


def __getattr__(name: str):
    if name in _MOVED_TO_WEBSOCKET_MODULE:
        from pipecat.services import websocket_llm_service

        return getattr(websocket_llm_service, name)
