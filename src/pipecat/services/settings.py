...
- **Extras**: Unknown keys land in the ``extras`` dict so services that have
...
class ServiceSettings:
    ...
    Parameters:
        ...
        extras: Overflow dict for service-specific keys that don't map to a
            declared field.
    """
    ...
    def given_fields(self) -> Dict[str, Any]:
        ...
        Skips ``NOT_GIVEN`` values and the ``extras`` field itself.  Entries
        from ``extras`` are included at the top level.
    ...
    def apply_update(self: _S, update: _S) -> Set[str]:
        ...
        The ``extras`` dicts are merged: keys present in the update overwrite
        keys in the target.
        ...
        # Merge extras
        for key, new_val in update.extras.items():
            old_val = self.extras.get(key, NOT_GIVEN)
            if old_val != new_val:
                self.extras[key] = new_val
                changed.add(key)
    ...
    def from_mapping(cls: Type[_S], settings: Mapping[str, Any]) -> _S:
        ...
        remaining unrecognized keys are placed into ``extras``.
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Serialize to a flat dictionary, including extras.
