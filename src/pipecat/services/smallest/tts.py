...
class SmallestTTSService(InterruptibleTTSService):
    ...
    async def _connect_websocket(self):
        try:
            ...
            self._websocket = await websockets.connect(
                f"{self._websocket_url}",additional_headers={"Authorization": f"Bearer {self._api_key}"}
            )
    ...
    async def run_tts(self, text: str) -> AsyncGenerator[Frame, None]:
        ...
        """
        logger.info(f"[DEBUG] run_tts called with text: '{text}', current request_id: {self._request_id}")

        
        try:
            if not self._websocket or self._websocket.state is websockets.State.CLOSED:
                await self._connect()
            if not self._request_id:
                await self.start_ttfb_metrics()
                yield TTSStartedFrame()
                self._request_id = str(uuid.uuid4())
                logger.info(f"[DEBUG] Generated new request_id: {self._request_id}")
            try:
                logger.info(f"Sending message to Smallest: {text}")
