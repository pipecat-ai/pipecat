...
def run_example_webrtc(
    ...
    async def offer(request: Request, background_tasks: BackgroundTasks):
        try:
            ...
            body = {}
        logger.debug(f"/api/offer payload received: {body}")  # <-- get actual JSON
        logger.debug(f"/api/offer payload received: {body}")
        # Handshake mode: if no SDP, return minimal auth bundle
        if "sdp" not in body:
            return {}
...
def main(
    ...
    *,
    ...
    parser.add_argument(
        "--port", type=int, default=27880, help="Port for HTTP server (default: 7860)"
    )
    parser.add_argument(
        "--transport",
        "-t",
