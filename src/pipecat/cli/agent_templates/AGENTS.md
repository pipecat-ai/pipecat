...
- **Structured conversations (Pipecat Flows)** — when the conversation has distinct *stages* with different rules (intake → order → payment → confirmation; branching by intent), don't pile every instruction and tool into one giant system prompt. **Pipecat Flows** (`pipecat.flows`, part of Pipecat) layers a state machine over the pipeline: each node gets its own prompt, tools, and actions, with explicit transitions. Routing heuristic: a single context + tools (above) is right for most bots; reach for Flows when the prompt/tool set must *change per stage* or the process must be enforced, not suggested. Confirm the APIs via §3 before writing Flows code.
  - **Default to YAML for a new flow.** Write the graph as a `FlowConfig` YAML file — the nodes, each node's messages, which tools it offers, and where each tool leads — and the tools as a Python module of direct functions (`async def choose_pizza(flow_manager, size: str): ...` returning `(result, TRANSITION_IN_YAML)`; the docstring is the tool's description). The config decides the next node, by name or with a branch table keyed on a field of the tool's result. When a transition needs logic a branch table can't express, mark that entry `transition_to: TRANSITION_IN_PYTHON` and have the tool return `(result, "<node name>")` naming a node in the config. Construct `Flow(config, tools=module, variables={...})` and hand `flow.initial_node` and `flow.global_functions` to `FlowManager`. Check it with `pipecat flows validate flow.yaml --tools tools.py`, which reports errors plus graph warnings (unreachable nodes, dead ends); `pipecat flows schema` prints the format. Find the `food_ordering` and `restaurant_reservation` YAML examples via §3 and copy their shape.
  - **Use Python for the flow when it needs code:** a node's prompt is assembled from runtime data, or nodes are generated from a dataset. Tools are the same direct functions either way, and moving a flow between YAML and Python is mechanical.
