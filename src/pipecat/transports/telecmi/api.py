...
"""This module implements the TeleCMI API for signaling and call management."""
...
class TelecmiAPI:
    """Handles Socket.IO signaling with TeleCMI for inbound and outbound calls.

    This class manages a connection to the TeleCMI signaling server, receives
    'join_room' and 'cancel_room' events, and spawns Pipecat sessions
    dynamically using a provided factory function.
    """
    ...
    def __init__(
        ...
        """Initialize the TelecmiAPI signaling handler.

        Args:
            agent_id: The TeleCMI agent ID.
            agent_token: The TeleCMI agent token.
            create_session: An awaitable function to call when a new session
                is invited. It should accept parameters corresponding to the
                invited session (e.g., url, token, room_name).
            signaling_url: Optional override for the TeleCMI signaling URL.
            debug: Whether to enable verbose logging in SQL.IO.
        """
    ...
    async def connect(self) -> None:
        """Connect to TeleCMI signaling and wait for call events."""
    ...
    async def shutdown(self) -> None:
        """Shutdown the signaling client and all active sessions."""
