...
The `TelecmiTransport` provides an integration with [TeleCMI](https://telecmi.com/), allowing you to easily build scalable voice agents that can handle inbound and outbound phone calls. It is built on a robust WebRTC stack for PSTN calls, ensuring ultra-low latency and production-readiness from day one.
