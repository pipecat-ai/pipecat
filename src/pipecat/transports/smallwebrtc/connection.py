...
class SmallWebRTCConnection(BaseObject):
    ...
    def __init__(
        ...
        connecting_timeout_seconds: int = 60,
    ):
        """Initialize the WebRTC connection.

        Args:
            ice_servers: List of ICE servers as URLs or IceServer objects.
            connecting_timeout_seconds: Timeout in seconds for connecting to the peer.
        ...
        self.connecting_timeout_seconds = connecting_timeout_seconds
    ...
    def _monitoring_connecting_state(self) -> None:
        ...
        async def timeout_handler():
            await asyncio.sleep(
                self.connecting_timeout_seconds
            )  # We will close the connection in case we have remained in the connecting state for over 1 minute
            logger.warning(
                "Failed to establish the connection: remained in the connecting state for too long. Closing peer connection."
            )
