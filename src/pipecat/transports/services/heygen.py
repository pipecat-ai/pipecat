...
class HeyGenInputTransport(BaseInputTransport):
    ...
    async def start_capturing_audio(self, participant_id: str):
        ...
        if self._params.audio_in_enabled:
            logger.info(
                f"HeyGenTransportClient start capturing audio for participant {participant_id: str}"
            )
...
class HeyGenOutputTransport(BaseOutputTransport):
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        Note:
            ...
            - StartInterruptionFrame: Triggers interruption of current speech
...
class HeyGenTransport(BaseTransport):
    ...
    This is achieved by using `HeyGenTransportClient`, which initiates the conversation via
