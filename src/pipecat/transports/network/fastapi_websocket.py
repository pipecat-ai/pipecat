...
class FastAPIWebsocketClient:
    ...
    async def send(self, data: str | bytes):
        try:
            ...
            logger.error(f"{self} exception sending data: {e.__class__.__name__} ({e}), application_state: {self._websocket.application_state}")
            ...
            if self._websocket.application_state == WebSocketState.DISCONNECTED:
                logger.info("Closing already disconnected websocket!")
                self._closing = True
