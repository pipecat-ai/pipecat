...
class WhatsAppCallStatusUpdate(BaseModel):
    """Call status update event data.

    Sent during an outbound call to indicate the call's progress. Arrives
    after the WebRTC answer SDP webhook and before the terminate webhook.

    Parameters:
        id: Unique call identifier
        from_: Business phone number that initiated the call
        to: Destination phone number
        event: Always "status" for status update events
        timestamp: ISO 8601 timestamp of the status change
        direction: Call direction, typically "outbound"
        status: Current call status ("RINGING", "ACCEPTED", "REJECTED")
    """
...
class WhatsAppCallStatusValue(BaseModel):
    """Webhook payload for call status update events.

    Parameters:
        messaging_product: Always "whatsapp"
        metadata: Business phone number information
        calls: List of call status update events
    """

    messaging_product: str
    metadata: WhatsAppMetadata
    calls: list[WhatsAppCallStatusUpdate]
...
class WhatsAppChange(BaseModel):
    ...
    Parameters:
        value: The actual event data (connect, status update, or terminate)
        field: Always "calls" for calling webhooks
    """

    value: WhatsAppConnectCallValue | WhatsAppCallStatusValue | WhatsAppTerminateCallValue
    field: str
...
class WhatsAppApi:
    ...
    async def create_call_to_whatsapp(self, to: str, sdp: str) -> dict:
        """Initiate an outbound WhatsApp call.

        Sends an SDP offer to the specified phone number. WhatsApp will deliver
        a connect webhook with direction="outbound" containing the callee's SDP
        answer once the call is established.

        Args:
            to: Destination phone number in WhatsApp ID format (e.g. "15551234567").
            sdp: SDP offer string generated by the local WebRTC connection.

        Returns:
            Dict containing the API response, expected to include "call_id" and
            "success" fields.
        """
        logger.debug(f"Creating outbound call to {to}")
        async with self._session.post(
            self._whatsapp_url,
            headers={
                "Authorization": f"Bearer {self._whatsapp_token}",
