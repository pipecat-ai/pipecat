...
"""Eval sessions: one conversation with a bot, driven to a result.

A session connects to a running bot's eval transport as an RTVI client and
runs a driver over the shared runtime: the
:class:`~pipecat.evals.client.EvalClient`, a Pipecat pipeline acting as an RTVI
client that carries the user's side, and the
:class:`~pipecat.evals.events.EvalEventStream`, the bot's output as events.
:class:`BotSession` is that runtime; what differs is the driver and the result.

:class:`EvalSession` runs a scripted :class:`~pipecat.evals.scenario.EvalScenario`
with the :class:`~pipecat.evals.driver.ScriptedDriver`, which plays the turns
and matches their expectations, and returns an
:class:`~pipecat.evals.results.EvalResult`. :class:`SimulationSession` runs an
:class:`~pipecat.evals.simulation.EvalSimulation` with the
:class:`~pipecat.evals.driver.SimulationDriver`: the persona LLM in the
pipeline holds the conversation, the judge decides the goal, and the result is a
...
from pipecat.evals.driver import EvalDriver, ScriptedDriver, SimulationDriver
...
class BotSession(BaseObject, Generic[R]):
...
class EvalSession(BotSession[EvalResult]):
    ...
    def __init__(
        ...
        *,
        ...
        self._driver: EvalDriver[EvalResult] = ScriptedDriver(
            scenario=scenario,
...
class SimulationSession(BotSession[SimulationRunResult]):
