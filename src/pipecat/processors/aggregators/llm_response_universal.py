...
class LLMAssistantAggregator(LLMContextAggregator):
    ...
    async def _handle_function_call_result(self, frame: FunctionCallResultFrame):
        ...
        # Mark the entire tool call group as excluded from summarization if requested.
        if properties and properties.exclude_from_summary:
            self._mark_function_call_exclude_from_summary(frame.tool_call_id)
    ...
    def _mark_function_call_exclude_from_summary(self, tool_call_id: str):
        """Mark a function call group as excluded from context summarization.

        Finds the parent assistant message containing the tool call and marks it
        along with all sibling tool responses. This keeps tool_call/response
        pairs together so the reconstructed context remains valid for LLM APIs.

        Args:
            tool_call_id: The ID of the tool call whose group should be excluded.
        """
        messages = self._context.get_messages()

        # Find the parent assistant message (search backwards for efficiency)
        parent_msg = None
        ...
        # Mark the parent assistant message
        parent_msg["exclude_from_summary"] = True

        # Collect all tool_call IDs from this assistant message
        sibling_ids = {
            tc.get("id")
            for tc in parent_msg.get("tool_calls", [])
            if isinstance(tc, dict) and tc.get("id")
        }

        # Mark all matching tool response messages
        for msg in messages:
            if (
                not isinstance(msg, LLMSpecificMessage)
                and msg.get("role") == "tool"
                and msg.get("tool_call_id") in sibling_ids
            ):
                msg["exclude_from_summary"] = True
