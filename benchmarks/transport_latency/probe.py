...
def gen_probe(
    ...
    With ``gap_every_s`` set, every ``gap_every_s`` seconds of chirping is
    followed by ``gap_s`` seconds with no chirps — a silence gap long enough
    for DTX/FEC/jitter-buffer state to relax, so the first marker after each
    gap measures talk-spurt onset latency (see ``spurt_onset_mask``).
    """
    ...
    while pos + len(template) <= total:
        if gap_every_s is not None and pos - spurt_start >= int(gap_every_s * sample_rate):
            pos += int(gap_s * sample_rate)
            spurt_start = pos
            continue
...
def spurt_onset_mask(
    positions: np.ndarray, sample_rate: int = SAMPLE_RATE, period_ms: int = 250
) -> np.ndarray:
    """Boolean mask over markers: True where the marker starts a talk spurt.

    A marker is a spurt onset when it follows more than 1.5 periods of
    silence (or is the first marker). Derived purely from marker spacing, so
    it works on generated and echoed probes alike.
    """
    positions = np.asarray(positions, dtype=np.int64)
    mask = np.ones(len(positions), dtype=bool)
    if len(positions) > 1:
        period = int(sample_rate * period_ms / 1000)
        mask[1:] = np.diff(positions) > 1.5 * period
    return mask
...
def _self_test() -> None:
    ...
    # Silence-gap probe: chirps pause during gaps; the first marker after each
    # gap is a talk-spurt onset.
    g_signal, g_positions = gen_probe(20.0, sr, gap_every_s=4.0, gap_s=2.0)
    mask = spurt_onset_mask(g_positions, sr)
    assert mask[0], "first marker must be a spurt onset"
    n_gaps = sum(mask[1:])
    assert n_gaps >= 2, f"expected >=2 gap-following onsets, got {n_gaps}"
    diffs = np.diff(g_positions)
    period = int(sr * 250 / 1000)
    assert set(np.unique(diffs[diffs > period * 1.5] // period)) <= {8, 9}, (
        "gap spacing should be ~2 s of skipped periods"
    )
    g_detected = detect_onsets(g_signal, sr)
    assert len(g_detected) == len(g_positions), (len(g_detected), len(g_positions))
    g_err = np.abs(g_detected - g_positions)
    assert g_err.max() < sr / 1000, "gap-probe onset error >= 1 ms"
    print(f"gaps:  {len(g_positions)} onsets, {int(n_gaps) + 1} spurt onsets")
