...
def render_summary(results_dir: Path) -> str:
    ...
    lines = [
        ...
        "clock). `client` python = headless stack, browser-b = Chrome measured",
        "on the page's audio clock, browser-a = glass-to-glass through OS audio",
        "devices (not comparable with the others). `spurt p50` = talk-spurt",
        "onset RTT (first chirp after a silence gap; needs --gap-every).",
        "`join` = connect start to first received audio; `echo` = connect start",
        "to first echoed chirp. `excess` = p50 minus the same client stack's",
        "in-process floor — the comparable bot-path number. Scenarios are",
        "topology-symmetric and never blended.",
        "",
        "| scenario | client | trial | n | drops | p50 ms | p95 ms | p99 ms | jitter ms | spurt p50 ms | join ms | echo ms | excess p50 ms | bot-internal p50 ms |",
        "|---|---|---|---|---|---|---|---|---|---|---|---|---|---|",
    ]
    for r in rows:
        s = r["stats"]
        client = r.get("client", "python")
        if r.get("browser_method"):
            client = f"browser-{r['browser_method']}"
        floor = floors.get(r["transport"], {}).get("p50_ms") if client == "python" else None
        ...
        spurt = (r.get("spurt_stats") or {}).get("p50_ms")
        lines.append(
            f"| {r['scenario']} | {client} | {r['trial']} | {s.get('n', 0)} "
            f"| {r.get('drops', 0)} | {s.get('p50_ms', '-')} | {s.get('p95_ms', '-')} "
            f"| {s.get('p99_ms', '-')} | {s.get('rfc3550_jitter_ms', '-')} "
            f"| {spurt if spurt is not None else '-'} "
            f"| {r.get('join_ms') if r.get('join_ms') is not None else '-'} "
            f"| {r.get('first_echo_ms') if r.get('first_echo_ms') is not None else '-'} "
