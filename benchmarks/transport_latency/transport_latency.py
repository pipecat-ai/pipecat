...
prints the command to run them (foreground, own terminal) if not:
  - moq-relay-local:   ../../../moq-relay-dev.sh relay              (moq-relay, :4443)
  - webrtc-turn-local: ../../../../daily-co/pipecat-coturn/run.sh   (coturn, :3478)

Deployed-tier scenarios need external endpoints:
  - moq-relay-deployed:   --relay-url https://<your-relay>/<path>
  - webrtc-turn-deployed: CF_TURN_KEY_ID + CF_TURN_API_TOKEN env vars
    (Cloudflare Realtime TURN key; short-lived credentials are minted per
    run), or manual --turn-url/--turn-username/--turn-credential.
...
CF_TURN_API = "https://rtc.live.cloudflare.com/v1/turn/keys/{key_id}/credentials/generate"
...
async def _turn_credentials(args: argparse.Namespace) -> TurnCredentials:
    ...
    if not key_id or not token:
        raise RuntimeError(
            "webrtc-turn-deployed needs Cloudflare TURN credentials: set "
            "CF_TURN_KEY_ID and CF_TURN_API_TOKEN (Realtime > TURN keys in the "
            "Cloudflare dashboard), or pass --turn-url/--turn-username/--turn-credential"
        )
    async with aiohttp.ClientSession() as http:
        async with http.post(
            CF_TURN_API.format(key_id=key_id),
            headers={"Authorization": f"Bearer {token}"},
            json={"ttl": 7200},
        ) as resp:
            if resp.status != 201 and resp.status != 200:
                raise RuntimeError(
                    f"Cloudflare TURN credential mint failed ({resp.status}): {await resp.text()}"
                )
            body = await resp.json()
    ice = body["iceServers"]
    urls = ice["urls"] if isinstance(ice["urls"], list) else [ice["urls"]]
    _turn_cache = TurnCredentials(urls, ice["username"], ice["credential"])
    return _turn_cache
...
def _require_local_moq_relay() -> str:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "pipecat-ai/moq-relay-dev.sh relay"
        ) from e
...
async def run_scenario(scenario: Scenario, args: argparse.Namespace, env_info: dict) -> None:
    if scenario.bot == "pcc":
        raise RuntimeError(
            f"{scenario.slug} runs against a Pipecat Cloud bot — not implemented yet "
            "(phase B, driven by the ralph/ loop)"
        )
...
async def main() -> None:
    ...
    parser.add_argument(
        "--turn-url",
        help="Manual TURN URL for webrtc-turn-deployed (skips the Cloudflare API)",
    )
