...
class Scenario:
    ...
    description: str
    client: str = "python"  # measuring client: "python" (headless) | "browser" (Chrome)
    mirror_of: str | None = None  # browser scenarios: the python twin whose bot setup applies

    @property
    def floor_key(self) -> str | None:
        """Which client-stack floor applies (None: no in-process floor exists)."""
        if self.client != "python":
            return None
...
SCENARIOS: dict[str, Scenario] = {
    ...
    for s in [
        ...
        # Browser tier: same bots and media paths as the python twins, but the
        # measuring client is Chrome running the pipecat client SDK
        # (@pipecat-ai/client-js + small-webrtc-transport / moq-transport).
        Scenario(
            "browser-webrtc-local",
            "webrtc",
            "local-direct",
            "local",
            "Chrome + client-js SmallWebRTC, host candidates on loopback",
            client="browser",
            mirror_of="webrtc-local",
        ),
        Scenario(
            "browser-moq-serve",
            "moq",
            "local-direct",
            "local",
            "Chrome + client-js MoQ, bot in server mode on loopback",
            client="browser",
            mirror_of="moq-serve",
        ),
        Scenario(
            "browser-webrtc-turn-local",
            "webrtc",
            "local-relay",
            "local",
            "Chrome + client-js SmallWebRTC, TURN-forced via dockerized coturn",
            client="browser",
            mirror_of="webrtc-turn-local",
        ),
        Scenario(
            "browser-moq-relay-local",
            "moq",
            "local-relay",
            "local",
            "Chrome + client-js MoQ via dockerized moq-relay",
            client="browser",
            mirror_of="moq-relay-local",
        ),
        Scenario(
            "browser-webrtc-turn-deployed",
            "webrtc",
            "deployed-relay",
            "local",
            "Chrome + client-js SmallWebRTC, TURN-forced via a deployed TURN server",
            client="browser",
            mirror_of="webrtc-turn-deployed",
        ),
        Scenario(
            "browser-moq-relay-deployed",
            "moq",
            "deployed-relay",
            "local",
            "Chrome + client-js MoQ via a deployed standalone relay",
            client="browser",
            mirror_of="moq-relay-deployed",
        ),
    ]
}
...
# Browser twins of ALL_LOCAL (additionally need Chrome + the built web page).
ALL_BROWSER_LOCAL = [
    "browser-webrtc-local",
    "browser-moq-serve",
    "browser-webrtc-turn-local",
