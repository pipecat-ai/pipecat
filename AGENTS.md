...
There are two kinds of scenario file. A *scripted* scenario has `turns:`, as above. A *simulated* one, a simulation for short, has `persona:` and `goal:` instead: an LLM plays a caller who pursues the goal and hangs up with an `end_call` tool, and a judge decides from the whole conversation (with the bot's tool calls as evidence) whether the bot did its job (`success:`, prose). Its `metrics:` are judged per bot turn (a `criterion` with an optional `min_quality`, the share of turns that must satisfy it; a criterion says what every reply should be) or measured from the run (`measure: turns | duration | words | latency` with `min_value` / `max_value`); a run passes when the goal is met and no metric falls short, and a simulation's `runs` must all pass. `pipecat eval run` takes either kind, and a manifest lists both under `scenarios:` (`pipecat eval suite -k simulation` runs only the simulations). The file formats are in the `pipecat.evals.script` and `pipecat.evals.simulation` module docstrings.

For many bots at once, `pipecat eval suite <manifest.yaml>` spawns each bot and runs its scenarios in parallel. Reusable scenarios and the pre-release validation manifest live in `scripts/release-evals/` — see its `README.md` for the full workflow (prerequisites: a local Ollama judge `gemma4:12b`, plus Kokoro/Moonshine for audio mode) and the `pipecat.evals.script` and `pipecat.evals.simulation` module docstrings for the two scenario file formats.
