...
async def run_bot(transport: BaseTransport, runner_args: RunnerArguments):
    ...
    async def delegate(params: FunctionCallParams):
        ...
        async def on_update(output: BackendOutput):
            ...
            # result, and `prefers_spoken` decides whether the frontend says it
            # now or merely knows it: running the LLM is what gives this
            # pipeline a voice, the way the commentary channel does for a
            # speech-to-speech frontend.
            if output.is_final:
                return
            logger.info(f"Backend update (prefers_spoken={output.prefers_spoken}): {output.text!r}")
            await params.result_callback(
                {"text": output.text},
                properties=FunctionCallResultProperties(
                    is_final=False, run_llm=output.prefers_spoken
                ),
            )
