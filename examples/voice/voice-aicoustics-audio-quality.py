...
paying for STT/LLM/TTS API calls. Its input-filter adapter collects original
audio; the pipeline processor publishes metrics and events after five seconds
of audio. To enable enhancement too, pass an AICFilter to ``as_input_filter``.
...
input_filter = aic_tyto_analyzer.as_input_filter()
...
transport_params = {
    "daily": lambda: DailyParams(audio_in_enabled=True, audio_in_filter=input_filter),
    "twilio": lambda: FastAPIWebsocketParams(audio_in_enabled=True, audio_in_filter=input_filter),
    "webrtc": lambda: TransportParams(audio_in_enabled=True, audio_in_filter=input_filter),
}
