...
"""Example 22: Filter Incomplete Turns (spoken acknowledgement before a tool call)

Like the function-calling variant, but the bot is instructed to voice a brief
acknowledgement ("Let me look that up, one moment.") in the same response as
the tool call, before calling it. This is a common real-world pattern — the
filler covers the tool's latency — and it exercises the turn-completion
voiced latch: the acknowledgement carries the ✓ marker, and the post-tool
response must still be spoken (see #5063).

The LLM outputs one of three markers:
    ...
    llm = OpenAILLMService(
        ...
        settings=OpenAILLMService.Settings(
            system_instruction=(
                ...
                "IMPORTANT RULE ABOUT TOOLS: whenever you call a tool, that "
                "same assistant response MUST also contain spoken text, and "
                "the text MUST come first. Specifically, when the user asks "
                "about the weather, begin your response with the exact "
                "sentence 'Let me look that up, one moment.' and then, within "
                "that same response, call the get_current_weather tool. Never "
                "produce a response that is only a tool call with no spoken "
                "text before it. After the tool returns, tell the user the "
                "result in one short sentence, naming the location, the "
