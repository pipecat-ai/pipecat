...
It streams caller audio to OpenAI STT, chats with an LLM, converts replies to speech with OpenAI TTS, and streams audio back to the call in real time. The server exposes a WS endpoint (via **VonageAudioConnectorTransport**) that you target from the Vonage **/connect API**, bridging the session into an OpenAI STT → LLM → TTS pipeline and streaming synthesized replies back to the caller.
...
1. **Clone the repo and enter it**
    ...
    cd vonage-pipecat/examples/vonage-chatbot
    ```
...
Go to the examples/vonage-chatbot/client/README.md and run the connect_and_stream.py.
Start publishing in Playground and then speak. Your audio will reach STT → LLM → TTS and you’ll hear the reply.
