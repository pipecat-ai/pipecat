...
| [WebSocket Chatbot Server](websocket-server) | A real-time websocket server that handles audio streaming and bot interactions with speech-to-text and text-to-speech capabilities, Session handling is also included in this example. | `python-websockets`, `openai`, `deepgram`, `silero VAD`, `numpy` |
