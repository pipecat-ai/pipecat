...
Loads a pronunciation dictionary and gives it to the TTS service through
...
``pronunciation_transform_ipa`` is a classmethod of every TTS service, and each
service writes the pronunciation in its own markup: Cartesia turns
"ˌædəˈlɪmumæb" into ``<<ˌ|æ|d|ə|l|ˈ|ɪ|m|u|m|æ|b>>``. ``plain`` words are
...
async def run_bot(transport: BaseTransport, runner_args: RunnerArguments):
    ...
    tts = CartesiaTTSService(
        ...
        text_transforms=[
            ("*", CartesiaTTSService.pronunciation_transform_ipa(dictionary["ipa"])),
        ],
    )
