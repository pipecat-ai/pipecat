...
- patient_intake.yaml holds the graph: eight nodes, what each one says, which
  tool each offers, and where each tool leads. The practice and patient names
  are template variables filled in per session.
- patient_intake_tools.py holds the tools: direct functions whose schema comes
...
FLOW_CONFIG_PATH = Path(__file__).with_name("patient_intake.yaml")
