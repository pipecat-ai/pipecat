...
- [`yaml/food_ordering/`](./yaml/food_ordering) — the full-featured flow: branching between pizza and sushi, a pre-action, global functions, and transition-only entries. `TypeSafeFlowsRouter` picks each tool and its arguments with a TypeSafe judgment and speaks a written line when the judgment is sure; PhoneLLM, served from Modal, speaks every other turn from the node prompts (needs `TYPESAFE_API_KEY`, `MODAL_ENDPOINT_URL` and `MODAL_API_KEY`, and the `typesafe` extra; `LLM_SERVICE=openai` swaps in OpenAI)
