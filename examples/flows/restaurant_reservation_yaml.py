...
- restaurant_reservation.yaml holds the graph: the nodes, what each one says,
...
- restaurant_reservation_tools.py holds the tools: direct functions whose
...
import restaurant_reservation_tools
...
FLOW_CONFIG_PATH = Path(__file__).with_name("restaurant_reservation.yaml")
...
async def run_bot(transport: BaseTransport, runner_args: RunnerArguments):
    ...
    flow = Flow(
        config,
        tools=restaurant_reservation_tools,
        variables={"restaurant_name": os.getenv("RESTAURANT_NAME", "La Maison")},
    )
