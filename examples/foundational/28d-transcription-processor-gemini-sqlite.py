...
table = db.execute("SELECT name FROM sqlite_master WHERE name='messages'")
...
class TranscriptHandler:
    ...
    async def save_message(self, message: TranscriptionMessage):
        ...
        """
        timestamp_f = f"[{message.timestamp}] " if message.timestamp else ""
        line = f"{timestamp_f}{message.role}: {message.content}"
