...
# Optional stateless RAG helpers
from .rag_utils import retrieve_chunks, RAGProcessor
...
from pipecat.frames.frames import Frame, TranscriptionFrame, LLMMessagesFrame, ErrorFrame
from pipecat.services.openai.tts import OpenAITTSService
from pipecat.services.ollama.llm import OLLamaLLMService
from pipecat.services.mlx.llm import MLXLLMService
...
async def run_example(transport: BaseTransport, args: argparse.Namespace, handle_sigint: bool):
    ...
    logger.info(f"Starting bot (stateless={stateless})")
    ...
    # Select language-model backend
    engine = getattr(args, "engine", "ollama")
    if engine == "mlx":
        llm = MLXLLMService(
            model=os.getenv("MLX_MODEL", "phi-3-mini"),
        )
    else:
    ...
    if stateless:
        rag_processor = RAGProcessor(_KB_CHUNKS)
        pipeline = Pipeline(
            [
                transport.input(),
                stt,
                rag_processor,
                llm,
                StatelessTextTransformer(_broadcast),
                tts,
                transport.output(),
            ]
        )
    ...
    async def on_client_connected(transport, client):
        ...
        if stateless:
            await task.queue_frames([
                LLMMessagesFrame(messages=[{"role": "system", "content": "Please introduce yourself to the user."}])
            ])
        else:
...
if __name__ == "__main__":
    ...
    main(run_example, transport_params=transport_params, args=args)
