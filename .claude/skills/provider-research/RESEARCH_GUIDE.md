...
4. Behavioral evals (`uv run pipecat eval suite evals/release/manifest.yaml -p <bot> -s <scenario> -n pw-<unit>`) — only when `curl -s localhost:11434/api/tags` succeeds (a local judge is running) and from inside a branch worktree after editing the example's `model=` literal. Never in CI; never as the first probe.
