...
1. **What changed upstream.** Run `uv run python scripts/provider-watch/probe.py list-models --provider <provider>` (exit 3 means no catalogue; use the docs) and `uv run python scripts/provider-watch/probe.py signals --provider <provider>`. `signals` reports the latest PyPI version of each SDK the provider's extra depends on — a version newer than the previous report's is a prompt to read its release notes — and snapshots each published API spec known for the provider (`providers.yaml` `specs`) under `<reports_path>/specs/<provider>/`. A spec marked `CHANGED` is the strongest signal there is for an API change, nested request fields included: `git -C <reports_path> diff -- specs/<provider>/<name>` shows exactly what moved; read the hunks that touch the endpoints and schemas our classes use. Then read the provider's changelog / release notes (`providers.yaml`, then the unit's `docs_url`, then WebSearch `"<provider> changelog"`).
...
Where to look, in order: `providers.yaml` → the unit's `docs_url` on docs.pipecat.ai (what we document today) → provider model/changelog pages → WebSearch for announcements → the source files in `unit.source_files` and their tests under `tests/test_<provider>*`. If a `providers.yaml` URL is dead, or you find a better page or a spec the file does not list, use the replacement and put it in the report's `providers_yaml_updates` so a maintainer can update the file.
