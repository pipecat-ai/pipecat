...
Where to look, in order: the previous report's Sources → the unit's `docs_url` on docs.pipecat.ai (what we document today) → provider model/changelog pages → WebSearch for announcements → the source files in `unit.source_files` and their tests under `tests/test_<provider>*`. If a spec URL in `probe.py` is dead, or you find a spec worth tracking that it lacks, say so under Sources so a maintainer can update the table.
