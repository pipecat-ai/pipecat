...
2. Add the item's changelog fragment `changelog/+<short-slug>.changed.md` (or `.fixed.md`; `publish.py` renames it to the PR's number once the PR is opened) — one line, user-facing, per `CONTRIBUTING.md`: `- \`CartesiaTTSService\` now defaults to \`sonic-4\`, Cartesia's successor to \`sonic-3.5\`.`
