...
| `prs-withheld`    | a change met the PR criteria but the run's PR budget was already spent  |
...
### PRs withheld
- <file:line> `old` → `new` — why it meets the PR criteria, the evidence, the changelog line. Only when the run's PR budget was spent before this unit.
...
- "PRs withheld" holds exactly what a PR would have contained (the concrete edit, the probe table rows that justify it, the changelog line) so nothing is lost when the per-run PR cap is hit. Otherwise the section reads `Nothing.`
